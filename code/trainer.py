...
class Trainer:
    def __init__(self, config, args=None):
        ...
        if (self.train_df is None or self.validation_df is None or
                (args and args.test and self.test_df is None)):
    ...
    def get_batch(self, data, start_index, batch_size):
        ...
        for key in data.keys():
            if isinstance(data[key], np.ndarray):
                if key.endswith('_ys'):
                    # The time series data is already a 2D NumPy array (thanks to padding in dataset.py)
                    batch[key] = data[key][start_index:end_index]
                elif key.endswith('_ems') or key.endswith('_cis'):  # Now we slice ems and cis
                    batch[key] = data[key][start_index:end_index]
                else:
                    batch[key] = data[key][start_index:end_index]
            else:
    ...
    def to_variable(self, data):
        ...
        for i in range(self.num_features):
            ...
            if data[f'{feature_name}_ems'] is not None:
                # Key change:  Directly use the loaded embeddings (they are already numpy arrays)
