...
class Trainer:
    def __init__(self, config, args=None):
        ...
        print(self.device)  # Print device for debugging
        ...
        # Create datasets by sampling. This happens *inside* the trainer.
        print("Sampling data...")
        ...
        if self.train_data['stock'].size == 0: # Check for valid data.
        ...
        # Load test data and embeddings/CI *only* if in test mode.
    ...
    def load_embeddings_and_ci(self, data, dataset_type):
        """Loads pre-computed embeddings and CI values *efficiently*."""

        # Dictionaries to cache loaded data: (stock, feature) -> data
        loaded_embeddings = {}
        loaded_ci = {}

        # Pre-allocate lists with None (critical for correct batching)
                ...
                if (stock, feature) not in loaded_ci:
                    if os.path.exists(ci_file):
                        try:
                            ...
                            loaded_ci[(stock, feature)] = {}  # Use empty dict on error
                    else:
    ...
    def get_batch(self, data, start_index, batch_size):
        ...
        for key in data.keys():
            ...
            elif isinstance(data[key], list):  # Handle lists (for embeddings/CI)
                batch[key] = data[key][start_index:end_index] # Correctly slices lists.
            else:
    ...
    def to_variable(self, data):
        ...
        for i in range(self.num_features):
            ...
            # Handle potentially None embeddings:
            if data[f'{feature_name}_ems'] is not None:
                ems_list = []
                for item in data[f'{feature_name}_ems']:
                    if item is not None:  # Handle None values in the list.
                        ems_list.append(item)
                    else:
                        # Handle the None case.  Pad with zeros.
                        ems_list.append(np.zeros((self.time_step, config['model']['embedding_dim']), dtype=np.float32))  # Use correct embedding dim!
                var_dict['ems'] = torch.tensor(np.stack(ems_list), dtype=torch.float32).to(self.device)


            # Handle potentially None CI values:
            if data[f'{feature_name}_cis'] is not None:
                cis_list = []
                for item in data[f'{feature_name}_cis']:
                    if item is not None:
                        cis_list.append(item)
                    else:
                        # Pad with zeros (or some other suitable default)
                        cis_list.append(np.zeros(self.time_step, dtype=np.float32)) # Correct time_step shape
                var_dict['cis'] = torch.tensor(np.stack(cis_list), dtype=torch.float32).to(self.device)
