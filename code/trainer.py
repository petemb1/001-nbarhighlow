    ...
    def load_embeddings_and_ci(self, data, dataset_type):
        ...
        for i in range(len(data['stock'])):  # Iterate through samples
            ...
            for feature in config['data']['features']:
                ...
                if (stock, feature) not in loaded_ci:  # <--- Check the cache FIRST
                    if os.path.exists(ci_file):
                        try:
                            ...
                            loaded_ci[(stock, feature)] = {}  # Set to empty dict on error
                    else:
                ...
                if date in loaded_ci.get((stock, feature), {}):  # Use get with default
                    ...
                    # Convert keys to integers for sorting, then get values as floats
    ...
    def to_variable(self, data):
        ...
        for i in range(self.num_features):
            ...
            var_dict = {
                ...
                'ys': torch.tensor(data[f'{feature_name}_ys'], dtype=torch.float32).unsqueeze(-1).to(self.device),  # Add dimension
